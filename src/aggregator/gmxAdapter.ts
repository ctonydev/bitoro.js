...
export function computeGmxAdapterOpenPosition(
  ...
  const { gmxCollateral, aggregatorCollateral, bitoroCollateral } = _loadCollateral(
    assets,
    storage,
    pos.collateralTokenAddress,
    pos.assetTokenAddress,
  ...
  if (borrowCollateral.gt(_0)) {
    if (!bitoroCollateral) {
      throw new InvalidArgumentError(`bitoro: can not borrow (${gmxCollateral.config.symbol}): no such token`)
    }
    if (bitoroCollateral.spotLiquidity.lt(borrowCollateral)) {
      if (!liquidityWarning) {
        liquidityWarning = new InsufficientLiquidityError(
          ...
          `can not borrow (${bitoroCollateral.symbol
          }): ${bitoroCollateral.spotLiquidity.toFixed()} < ${borrowCollateral.toFixed()}`
        )
      }
    }
...
function _computeAggregatorFundingFee(
  ...
  const { gmxCollateral, bitoroCollateral } = _loadCollateral(
    assets,
    storage,
    pos.collateralTokenAddress,
    pos.assetTokenAddress,
  ...
  if (
    pos.isLong &&
    pos.cumulativeDebt.gt(_0) /* note: some of gmxCollateral can not borrow. thus there is no bitoroCollateral */
  ) {
    if (!bitoroCollateral) {
  ...
  if (!pos.isLong) {
    ...
    if (!bitoroCollateral) {
      throw new InvalidArgumentError(`missing bitoroAsset[${storage.shortFundingAssetId}]`)
    }
    if (bitoroCollateral.isStable) {
      throw new InvalidArgumentError(`bad config. bitoroAsset[${storage.shortFundingAssetId}] can not be stable coin`)
    }
    const gmxShortFundingAsset = storage.gmx.tokens[bitoroCollateral.tokenAddress.toLowerCase()]
    if (!gmxShortFundingAsset) {
      throw new InvalidArgumentError(`missing gmxAsset[${bitoroCollateral.tokenAddress}]`)
    }
...
function _loadCollateral(
  ...
  let bitoroCollateral: Asset | undefined = undefined
  if (typeof bitoroCollateralId !== 'undefined') {
    // bitoro has this collateral
    bitoroCollateral = assets[bitoroCollateralId]
    if (!bitoroCollateral) {
      throw new InvalidArgumentError(`missing bitoroAsset[${bitoroCollateralId}]`)
    }
    if (collateralTokenAddress.toLowerCase() !== bitoroCollateral.tokenAddress.toLowerCase()) {
      throw new InvalidArgumentError(
        `pos.collateral(${collateralTokenAddress}) != pos.collateralId(${bitoroCollateralId})`
      )
    }
  } else {
